...
static u32 GetAppletPos(u32 attributes) {
    constexpr u32 AppletPosMask = 7;
    return attributes & AppletPosMask;
}
...
static AppletSlotData* GetAppletSlotData(AppletId id) {
    if (id == AppletId::Application) {
        auto& slot = applet_slots[static_cast<size_t>(AppletSlot::Application)];
    ...
    if (id == AppletId::AnySystemApplet) {
        auto& system_slot = applet_slots[static_cast<size_t>(AppletSlot::SystemApplet)];
        ...
        auto& home_slot = applet_slots[static_cast<size_t>(AppletSlot::HomeMenu)];
    ...
    if (id == AppletId::HomeMenu || id == AppletId::AlternateMenu) {
        auto& slot = applet_slots[static_cast<size_t>(AppletSlot::HomeMenu)];
        if (slot.applet_id != AppletId::None)
            return &slot;
...
void Enable(Service::Interface* self) {
    ...
    // Update the applet attributes
    if (slot_data->slot == AppletSlot::HomeMenu && slot_data->applet_id != AppletId::None) {
        slot_data->attributes |= attributes;
    }
