...
struct SelfNCCHFilePath {
    SelfNCCHFilePathType type;
    std::array<char, 8> exefs_filename;
};

static_assert(sizeof(SelfNCCHFilePath) == 12, "NCCHFilePath has wrong size!");

// A read-only file created from a block of data. It only allows reading the whole file from the
// begining to the end.
...
public:
    ...
    ResultVal<std::unique_ptr<FileBackend>> OpenFile(const Path& path, const Mode&) const override {
        ...
        case SelfNCCHFilePathType::Code:
            LOG_ERROR(Service_FS, "Reading code section is not supported!");
            return ERROR_COMMAND_NOT_ALLOWED;

        case SelfNCCHFilePathType::ExeFS: {
            std::string filename(file_path.exefs_filename.begin(), file_path.exefs_filename.end());
            const size_t pos = filename.find('\0');
...
private:
    ...
    ResultVal<std::unique_ptr<FileBackend>> OpenExeFS(const std::string& filename) const {
        if (filename == "icon") {
            ...
            LOG_ERROR(Service_FS, "Unable to read icon!");
        ...
        if (filename == "logo") {
            ...
            LOG_ERROR(Service_FS, "Unable to read logo!");
            return ERROR_EXEFS_SECTION_NOT_FOUND;
        }
        ...
        if (filename == "banner") {
            ...
            LOG_ERROR(Service_FS, "Unable to read banner!");
            return ERROR_EXEFS_SECTION_NOT_FOUND;
        }
