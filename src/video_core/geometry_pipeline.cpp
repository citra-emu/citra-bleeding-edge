...
// In the Point mode, vertex attributes are sent to the input registers in geometry shader unit.
...
// In VariablePrimitive mode, vertex attributes are buffered into the uniform registers in geometry
// shader unit. The number of vertex is variable, which is specified by the first index value in a
// batch. This mode is usually used for subdivision.
...
// In FixedPrimitive mode, vertex attributes are buffered into the uniform registers in geometry
