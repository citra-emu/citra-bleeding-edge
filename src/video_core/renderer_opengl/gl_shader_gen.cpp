...
float ProcTexNoiseCoef(vec2 x) {
    ...
    // Shift
    out += "float u = ";
    AppendProcTexShift(out, "uv.x", "uv_before_noise.y", config.state.proctex.u_shift,
                       config.state.proctex.u_clamp);
    out += ";\n";
    out += "float v = ";
    AppendProcTexShift(out, "uv.y", "uv_before_noise.x", config.state.proctex.v_shift,
