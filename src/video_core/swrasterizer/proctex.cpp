...
Math::Vec4<u8> ProcTex(float u, float v, TexturingRegs regs, State::ProcTex state) {
    ...
    const float u_before_noise = u;
    const float v_before_noise = v;
    ...
    // Shift
    const float u_shift = ShiftCoord(u, v_before_noise, regs.proctex.u_shift, regs.proctex.u_clamp);
    const float v_shift = ShiftCoord(v, u_before_noise, regs.proctex.v_shift, regs.proctex.v_clamp);
    u = u_shift;
    v = v_shift;
